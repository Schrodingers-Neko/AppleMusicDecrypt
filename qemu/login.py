...
from src.qemu import QemuInstance, kill_all_qemu  # noqa: E402
...
async def _lite_login_flow() -> int:
    ...
    # Ensure no lingering QEMU instance is holding port 12340
    kill_all_qemu()
    ...
    already_logged_in = await _regions_available()
    await qemu.terminate()
    ...
    print("Once submitted, Apple will send a Two-Factor Authentication (2FA) prompt")
    print("to your trusted devices. You can enter the 6-digit code in the console.\n")

    username = input("Apple ID / Username: ").strip()
    if not username:
    ...
    print("\nStarting wrapper-lite login in guest console...")
    print("Watch for the '2FA code: ' prompt when your device receives the alert.\n")
    ...
    print("\nVerifying cached account with wrapper-lite...")
    ...
    if ok:
        print("\nLogin successful! You can now start the app with start.bat or start.ps1.")
        return 0
